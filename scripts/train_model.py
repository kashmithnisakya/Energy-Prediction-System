...
def main(args):
    ...
    mlflow.set_experiment("Power and energy prediction")
    ...
    model_path = Path(f"../ml/model/{args.model_name}.h5")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train LSTM model for power and energy prediction"
    )
    parser.add_argument(
        "--csv_path", type=str, required=True, help="Path to processed CSV file"
    )
    parser.add_argument(
        "--window_size", type=int, default=672, help="Input window size (e.g. 14 days)"
    )
    ...
    parser.add_argument(
        "--model_name", type=str, default="model", help="Name of the model"
    )

...
def load_and_preprocess_data(csv_path, test_size_days=30):
    ...
    try:
        ...
        daily_df = df.groupby('date').agg({'y': 'sum'}).reset_index()
        daily_df = daily_df.rename(columns={'date': 'ds'})
        
        # Handle zeros and missing values in aggregated data
        first_valid = daily_df['y'].ne(0).idxmax()
        daily_df.loc[:first_valid, 'y'] = np.nan
        daily_df['y'] = daily_df['y'].replace(0, np.nan).ffill()
        
        # Train-test split
        split_date = daily_df['ds'].max() - timedelta(days=test_size_days)
...
def plot_results(model, forecast, test, save_dir="../ml/plots"):
    ...
    try:
        ...
        plt.plot(test['ds'], forecast.loc[forecast['ds'].isin(test['ds']), 'yhat'], label='Predicted')
...
def cross_validate_model(model, train, params):
    ...
    try:
        initial = params.get('cv_initial', '180 days')
        period = params.get('cv_period', '90 days')
...
def main(args):
    with mlflow.start_run():
        ...
        # Save full results
        results_path = Path(f"../ml/results/{args.model_name}_results.csv")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--country_holidays', type=str, default=None, help='Country code for holidays')
